...
Even the best model only explains a small portion of the variance in duration_seconds. The low R2 across all models tells that spatial and temporal variables alone are not strong predictors of how long a UFO sighting lasts. The duration of a report likely reflects more heavily on human memory instead of structured geographic or temporal patterns.

My analysis shows that predicting how long a UFO sighting lasts is highly unreliable using only the available information. The Random Forest model tells me that there are faint patterns that can be detected, suggesting that there are some external variables influencing sighting duration.
